...
def do_firewall(policy):
    rule = of.ofp_flow_mod()
    match = of.ofp_match()
    match.dl_type = int(0x0800)
    ...
    # Check nw_src and nw_dst (ip address)
    if policy['ip-src'] != '-':
        match.nw_src= policy['ip-src']
    if policy['ip-dst']!= '-':
        match.nw_dst = policy['ip-dst']

    # Check dl_src and dl_dst (mac address)

    if policy['mac-src'] != '-':
        match.dl_src = EthAddr(policy['mac_sr'])
    if policy['mac-dst'] != '-':
